...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        logger.debug("  specific  - Run specific test file (provide path as next arg)")
        sys.exit(1)

    mode = sys.argv[1]

    if mode == "all":
        return run_command(["pytest", "-v", "--cov=nano_banana", "--cov-report=term-missing"])
    ...
    if mode == "specific":
        if len(sys.argv) < 3:
            ...
            logger.debug("Example: python run_tests.py specific tests/core/test_config.py")
            sys.exit(1)
